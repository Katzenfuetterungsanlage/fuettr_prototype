...
\section{Umsetzung}
Bei der Umsetzung, also beim Schreiben des Programms, wurde wie folgt vorgegangen. Zuerst wurden alle \ac{GUI}-Fenster per Hand grob entworfen. Anschließend wurden diese in der IDE Netbeans als  \inlinecode{JavaStyle}{javax.swing.JFrame} Form erstellt. Genaueres über die \ac{GUI}-Fenster folgt unter Punkt 2.3.4. Danach wurden grundlegende Funktionen, die das Programm zu erfüllen hat, implementiert. Weiters wurden noch die Klassen: Mongodb, \ac{pi4j}, der Server und der ErrorAndWarningHandler als Singleton implementiert. 
\\ \\ 
...
\\ Befehle:
\begin{itemize} 
\item[•] \textbf{\inlinecode{JavaStyle}{show dbs}} ... anzeigen aller angelegten Datenbanken
\item[•] \textbf{\inlinecode{JavaStyle}{show collections}} ... anzeigen aller Collections in einer Datenbank
...
\item[•] \textbf{\inlinecode{JavaStyle}{drop()}} ... löschen einer Datenbank oder einer Collection
\\     (Beispiel: \inlinecode{JavaStyle}{<Datenbankname>.drop()} , \inlinecode{JavaStyle}{<Collectionname>.drop()})
\item[•] \textbf{\inlinecode{JavaStyle}{find()}} ... suchen nach bestimmten Daten 
\\     (Beispiel: \inlinecode{JavaStyle}{db.data.find()} )
\item[•] \textbf{\inlinecode{JavaStyle}{count()}} ... zählen der Dokumente in einer Collection
...
\\     (Beispiel: \inlinecode{JavaStyle}{db.data.updateOne(<ObjektID>, <Daten>)} )
\item[•] \textbf{\inlinecode{JavaStyle}{deleteOne()}} ... löschen eines Dokumentes
\\     (Beispiel: \inlinecode{JavaStyle}{db.data.deleteOne(<ObjektID>)} )
\end{itemize}
...
\begin{comment} 
 Beginnt der Request mit einem \textbf{PUT}, bedeutet das, dass der Client dem Server Daten schicken will.
...
GET                                   & /errors\_warnings \textbackslash r\textbackslash n                & Der Server schickt dem Client die aktuell anzuzeigenden Errors und Warnungen in einer Json-Array  \\ \hline
GET                                   & /ChangeMachineState \textbackslash r\textbackslash n              & Der Server ruft eine Methode auf um den Maschinenzustand zu ändern                               \\ \hline
\end{tabular}
...
\item[2] Danach wird überprüft, ob der Request "GET\grqq{} \begin{comment} oder "PUT\grqq{} \end{comment} enthält. Wenn keine dieser beiden Funktionen enthalten ist, wird eine Fehlermeldung gesendet.
...
\begin{itemize}
\item[•] \inlinecode{JavaStyle}{doInBackground()}
\\ Mit \textbf{doInBackground()} wird dem SwingWorker mitgeteilt welche Methoden im Hintergrund auszuführen sind. Wenn der Swingworker anschließend mit \inlinecode{JavaStyle}{.execute()} ausgeführt wird, wird ein neuer Hintergrund-Thread erstellt. Dieser Thread läuft parallel zum \ac{EDT}. Der Hintergrund-Thread wird  geschlossen, wenn mit \inlinecode{JavaStyle}{.cancel()} ein Flag, welches dem SwingWorker mitteilt, dass er sich demnächst beenden soll, gesetzt wird. Weiters kann sich der Hintergrund-Thread beenden, wenn alle Methoden abgearbeitet wurden oder im Thread eine Exception geworfen wird. In einem Hintergrund-Thread darf nicht auf die \ac{GUI}-Elemente von Java Swing zugegriffen werden. 
\\ Im folgenden Beispiel wird ein Hintergrund-Thread gezeigt, welcher durch das Beenden des SwingWorkers mit \inlinecode{JavaStyle}{.cancel(true)} beendbar ist.
...
\end{lstlisting} 
Wenn der SwingWorker beendet wird, während er aufgrund der Methode \\ \inlinecode{JavaStyle}{TimeUnit.MILLISECONDS.sleep(500);} wartet, wird eine \textbf{InterruptedException} geworfen. Dies ist nur der Fall, wenn der SwingWorker mit \inlinecode{JavaStyle}{.cancel(true)} beendet wird.
...
\item[•] cancel()
\\ Mit \inlinecode{JavaStyle}{.cancel(true)} kann ein SwingWorker beendet werden. Das \textbf{true} in den Klammern bedeutet, dass ein Thread-Interrupt an den Thread gesendet. Wenn statt \textbf{true false} verwendet wird, wird das Flag erst beim nächsten Schleifendurchlauf überprüft.
...
Um das Passwort einzugeben wird ein \textbf{PasswordField} verwendet. Dieses Eingabefeld wird speziell dazu verwendet Passwörter einzugeben. Eine Besonderheit davon ist, dass die Passwörter nicht als Text sondern als Sternchen dargestellt werden. Eine weitere Besonderheit ist, dass der Inhalt eine Zeichenkette ist, also ein Feld von Characters: \textbf{char[]}. Weiters unterscheidet sich auch das Abfragen des Inhaltes im Vergleich mit einem normalen Textfeld. Beim \textbf{PasswordField} kann der Inhalt mit \inlinecode{JavaStyle}{passwordField.getPassword()} abgefragt werden.
...
Um das Passwort zu hashen wird der Hashalgorithmus \textbf{SHA-512} verwendet. Im Programm wird das Passwort nur einmal gehasht. Falls die Katzenfütterungsanlage in Serie gehen sollte, sollte das Passwort öfters gehasht werden um die Sicherheit zu erhöhen.
...
\subsubsection{Probleme mit pi4j}\label{subsubsec:ProblemePi4j} 
